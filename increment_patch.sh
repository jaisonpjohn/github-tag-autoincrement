...
LAST_COMMIT=$(curl -sS \
                ...
                ${GITHUB_REPO_URL}/commits/master | jq -r '.sha')
echo "Last Commit: ${LAST_COMMIT}"
LAST_TAG=$(curl -sS \
                --user ${GITHUB_USERNAME}:${GITHUB_PASSWORD} \
                ${GITHUB_REPO_URL}/tags | jq -r '.[].name' | grep -m 1 ${TAG_PREFIX})
